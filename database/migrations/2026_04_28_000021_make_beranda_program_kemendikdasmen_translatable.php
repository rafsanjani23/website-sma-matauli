...
return new class extends Migration
{
    ...
    {
        Schema::table('beranda_program_kemendikdasmen', function (Blueprint $table) {
            $table->json('judul_tmp')->nullable()->after('judul');
            $table->json('ringkasan_tmp')->nullable()->after('ringkasan');
        });

        DB::table('beranda_program_kemendikdasmen')->orderBy('id')->chunk(100, function ($rows) {
            foreach ($rows as $row) {
                DB::table('beranda_program_kemendikdasmen')->where('id', $row->id)->update([
