...
return new class extends Migration
{
    ...
    {
        Schema::table('beranda_program_kemataulian', function (Blueprint $table) {
            $table->json('judul_tmp')->nullable()->after('judul');
            $table->json('ringkasan_tmp')->nullable()->after('ringkasan');
        });

        DB::table('beranda_program_kemataulian')->orderBy('id')->chunk(100, function ($rows) {
            foreach ($rows as $row) {
                DB::table('beranda_program_kemataulian')->where('id', $row->id)->update([
