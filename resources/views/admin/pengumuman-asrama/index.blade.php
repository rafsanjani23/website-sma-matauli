...
@section('title', 'Kelola Pengumuman Asrama')

@section('content')
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <h2 class="text-lg font-bold text-gray-900">Kelola Pengumuman Asrama</h2>

            <form action="{{ route('admin.pengumuman-asrama.index') }}" method="GET">
                <div class="flex rounded-full border-2 border-red-700 overflow-hidden max-w-md mx-auto">
                    ...
                    <button type="submit" aria-label="Cari pengumuman asrama"
                        class="flex items-center justify-center bg-red-800 hover:bg-red-700 px-6 cursor-pointer">
            ...
            <a href="{{ route('admin.pengumuman-asrama.create') }}"
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.pengumuman-asrama.edit', array_merge(['id' => $item->id], request()->query())) }}"
                                        class="text-blue-600 hover:text-blue-800 text-sm font-medium">Edit</a>
                                    <form
                                        action="{{ route('admin.pengumuman-asrama.destroy', array_merge(['id' => $item->id], request()->query())) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            class="text-red-600 hover:text-red-800 text-sm font-medium cursor-pointer">Hapus</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
