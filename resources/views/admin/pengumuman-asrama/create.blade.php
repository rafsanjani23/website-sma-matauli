...
@section('title', 'Tambah Pengumuman Asrama')
...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 class="text-lg font-bold text-gray-900 mb-6">Tambah Pengumuman Asrama</h2>

            @include('admin.pengumuman-asrama.partials.errors')

            <form action="{{ route('admin.pengumuman-asrama.store') }}" method="POST" enctype="multipart/form-data"
                class="space-y-5">
                @csrf
                @include('admin.pengumuman-asrama.partials.form')
            </form>
        </div>
    </div>
