...
@section('content')
    <div class="max-w-2xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 class="text-lg font-bold text-gray-900 mb-6">Tambah Admin Baru</h2>
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('admin.admin-users.store') }}" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label for="username" class="block text-sm font-semibold text-gray-700 mb-1.5">Username <span class="text-red-600">*</span></label>
                    <input type="text" name="username" id="username" value="{{ old('username') }}" required minlength="4" maxlength="50"
                    ...
                    <p class="mt-1 text-xs text-gray-500">Minimal 4 karakter. Hanya huruf, angka, underscore, dan strip.</p>
                </div>
                ...
                <div>
                    ...
                    <select name="role" id="role" required
                        ...
                        <option value="{{ \App\Models\Admin::ROLE_ADMIN_SEKOLAH }}" {{ old('role') === \App\Models\Admin::ROLE_ADMIN_SEKOLAH ? 'selected' : '' }}>Admin Sekolah (akses semua panel kecuali asrama)</option>
                        <option value="{{ \App\Models\Admin::ROLE_ADMIN_ASRAMA }}" {{ old('role') === \App\Models\Admin::ROLE_ADMIN_ASRAMA ? 'selected' : '' }}>Admin Asrama (hanya panel asrama)</option>
                    </select>
                </div>

                <div>
                    <label for="password" class="block text-sm font-semibold text-gray-700 mb-1.5">Password <span class="text-red-600">*</span></label>
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-semibold text-gray-700 mb-1.5">Konfirmasi Password <span class="text-red-600">*</span></label>
