...
@section('content')
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <form action="{{ route('admin.fasilitas-sekolah.index') }}" method="GET">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.fasilitas-sekolah.edit', array_merge(['id' => $item->id], request()->query())) }}"
                                        class="text-blue-600 hover:text-blue-800 text-sm font-medium">Edit</a>
                                    <form action="{{ route('admin.fasilitas-sekolah.destroy', array_merge(['id' => $item->id], request()->query())) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
