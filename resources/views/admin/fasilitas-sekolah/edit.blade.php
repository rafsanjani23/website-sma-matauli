...
@section('content')
    <div class="max-w-4xl">
        <div class="mb-6">
            <a href="{{ route('admin.fasilitas-sekolah.index', request()->query()) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-red-800 transition">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 class="text-lg font-bold text-gray-900 mb-6">Edit Fasilitas Sekolah</h2>
            ...
            <form action="{{ route('admin.fasilitas-sekolah.update', array_merge(['id' => $item->id], request()->query())) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="kategori" id="kategori" required
                        ...
                        <option value="Laboratorium / Studi" {{ old('kategori', $item->kategori) == 'Laboratorium / Studi' ? 'selected' : '' }}>Laboratorium / Studi</option>
                        <option value="Fasilitas Akademik" {{ old('kategori', $item->kategori) == 'Fasilitas Akademik' ? 'selected' : '' }}>Fasilitas Akademik</option>
                        <option value="Fasilitas Umum" {{ old('kategori', $item->kategori) == 'Fasilitas Umum' ? 'selected' : '' }}>Fasilitas Umum</option>
                    </select>
                </div>
