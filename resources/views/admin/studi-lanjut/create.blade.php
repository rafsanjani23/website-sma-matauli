...
@section('title', 'Tambah Studi Lanjut')
@section('content')
    <div class="max-w-4xl">
        <div class="mb-6">
            <a href="{{ route('admin.studi-lanjut.index', request()->query()) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-red-800 transition">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 class="text-lg font-bold text-gray-900 mb-6">Tambah Studi Lanjut</h2>
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('admin.studi-lanjut.store') }}" method="POST" class="space-y-5">
                @csrf
                <div>
                    <label for="nama_alumni" class="block text-sm font-semibold text-gray-700 mb-1.5">Nama Alumni</label>
                    <input type="text" name="nama_alumni" id="nama_alumni" value="{{ old('nama_alumni') }}" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition" />
                </div>
                <div data-lang-tabs>
                ...
                <div>
                    ...
                    <input type="text" name="angkatan" id="angkatan" value="{{ old('angkatan') }}" required placeholder="Contoh: 2024" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition" />
                </div>
                <div class="pt-4">
