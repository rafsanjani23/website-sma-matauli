...
@section('content')
    <div class="max-w-4xl">
        <div class="mb-6">
            <a href="{{ route('admin.testimoni.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-red-800 transition">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            ...
            <form action="{{ route('admin.testimoni.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div>
                    <label for="tahun_alumni" class="block text-sm font-semibold text-gray-700 mb-1.5">Tahun Alumni</label>
                ...
                <div>
                    ...
                    <textarea name="isi" id="isi" rows="4"
                        class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition">{{ old('isi', $item->isi) }}</textarea>
                </div>
