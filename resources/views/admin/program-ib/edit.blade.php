...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            ...
            <form action="{{ route('admin.program-ib.update', array_merge(['id' => $item->id], request()->query())) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                ...
                <div data-lang-tabs>
                    ...
                    <div data-tab-pane="id" class="space-y-5">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-1.5">Ringkasan (ID) <span class="font-normal text-gray-400">(ditampilkan di kartu)</span></label>
                            <textarea name="ringkasan[id]" rows="3"
                ...
                <div>
                    ...
                    @if ($item->gambar)
                        <div class="mb-2">
                            <img src="{{ Storage::url($item->gambar) }}" class="w-40 h-auto rounded-lg" />
                        </div>
                    @endif
                    <input type="file" name="gambar" id="gambar" accept="image/*" data-crop-free
                ...
                <div>
                    ...
                    @if ($item->gambar_opsional)
                        <div class="mb-2">
                            <img src="{{ Storage::url($item->gambar_opsional) }}" class="w-40 h-auto rounded-lg" />
                        </div>
                    @endif
