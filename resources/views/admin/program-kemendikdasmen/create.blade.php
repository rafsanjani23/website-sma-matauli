...
@section('title', 'Tambah Program Kemendikdasmen')

@section('content')
    <div class="max-w-4xl">
        <div class="mb-6">
            <a href="{{ route('admin.program-kemendikdasmen.index', request()->query()) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-red-800 transition">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 class="text-lg font-bold text-gray-900 mb-6">Tambah Program Kemendikdasmen</h2>
            ...
            <form action="{{ route('admin.program-kemendikdasmen.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                    ...
                    <div data-tab-pane="id" class="space-y-5">
                        ...
                        <div>
                            ...
                            <textarea name="isi_konten[id]" data-rich-editor rows="8" required
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition">{{ old('isi_konten.id') }}</textarea>
                        </div>
                    </div>
                    <div data-tab-pane="en" class="hidden space-y-5">
                        ...
                        <div>
                            ...
                            <textarea name="isi_konten[en]" data-rich-editor rows="8"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition">{{ old('isi_konten.en') }}</textarea>
                        </div>
                    </div>
                </div>
