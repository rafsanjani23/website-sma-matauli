...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="flex min-h-screen">
        ...
        {{-- Sidebar --}}
        <aside id="sidebar"
            class="fixed inset-y-0 left-0 z-50 w-64 bg-linear-to-b from-red-950 to-red-900 transform -translate-x-full lg:translate-x-0 transition-transform duration-300 lg:static lg:inset-auto">
            ...
            <div class="flex items-center gap-3 px-6 py-5 border-b border-red-800/50">
                <div class="w-10 h-10 bg-yellow-400/20 rounded-xl flex items-center justify-center">
                    <svg class="w-6 h-6 text-yellow-400" fill="none" stroke="currentColor" stroke-width="1.5"
                        viewBox="0 0 24 24">
        ...
        <div class="flex-1 flex flex-col min-h-screen lg:ml-0">
            ...
            {{-- Top Bar --}}
            <header
                class="bg-white shadow-sm border-b border-gray-200 px-4 md:px-6 py-3 flex items-center justify-between sticky top-0 z-30">
    ...
    <div id="confirm-modal" class="fixed inset-0 z-9998 hidden">
        <div class="absolute inset-0 bg-black/60 transition-opacity" id="confirm-overlay"></div>
        <div class="relative flex items-center justify-center min-h-screen p-4">
            <div
                class="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-[scaleIn_0.2s_ease-out]">
                <div class="p-6 text-center">
                    <div id="confirm-icon"
                        class="mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4"></div>
                ...
                <div class="flex border-t border-gray-200">
                    <button type="button" id="confirm-cancel"
                        class="flex-1 px-6 py-3.5 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition rounded-bl-2xl">Batal</button>
                    <button type="button" id="confirm-ok"
                        class="flex-1 px-6 py-3.5 text-sm font-semibold border-l border-gray-200 transition rounded-br-2xl"></button>
                </div>
            </div>
        </div>
    </div>
        ...
        (function() {
            ...
            const configs = {
                delete: {
                    ...
                    okText: 'Ya, Hapus',
                    okClass: 'text-red-600 hover:bg-red-50',
                },
                edit: {
                    icon: '<svg class="w-8 h-8 text-amber-600" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg>',
                    iconBg: 'bg-amber-100',
                    title: 'Simpan Perubahan',
                    message: 'Apakah Anda yakin ingin menyimpan perubahan data ini?',
                    okText: 'Ya, Simpan',
                    okClass: 'text-amber-600 hover:bg-amber-50',
                },
                create: {
                    icon: '<svg class="w-8 h-8 text-green-600" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 9v6m3-3H9m12 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>',
                    iconBg: 'bg-green-100',
                    title: 'Tambah Data',
                    message: 'Apakah Anda yakin ingin menambahkan data baru ini?',
                    okText: 'Ya, Tambah',
            ...
            function showConfirm(type, form) {
                ...
                pendingForm = form;
                iconEl.className = 'mx-auto w-16 h-16 rounded-full flex items-center justify-center mb-4 ' + config
                    .iconBg;
                ...
                okBtn.className =
                    'flex-1 px-6 py-3.5 text-sm font-semibold border-l border-gray-200 transition rounded-br-2xl ' +
                    config.okClass;
            ...
            document.addEventListener('submit', function(e) {
                ...
                if (method === 'DELETE') {
                    e.preventDefault();
                    showConfirm('delete', form);
                } else if (method === 'PUT' || method === 'PATCH') {
                    e.preventDefault();
                    showConfirm('edit', form);
                } else if (method === 'POST') {
                    var submitBtn = form.querySelector('button[type="submit"]');
                    if (submitBtn && submitBtn.textContent.trim() === 'Simpan') {
                        e.preventDefault();
                        showConfirm('create', form);
                    }
                }
            });
        })();
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200">
                    <button type="button" id="crop-cancel"
                        class="px-5 py-2.5 text-sm font-semibold text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition">Batal</button>
                    <button type="button" id="crop-confirm"
                        class="px-5 py-2.5 text-sm font-semibold text-white bg-red-800 rounded-xl hover:bg-red-900 transition">Crop
                        & Gunakan</button>
                </div>
            </div>
        </div>
        ...
        (function() {
            ...
            document.getElementById('crop-confirm').addEventListener('click', () => {
                ...
                canvas.toBlob(function(blob) {
                    ...
                    const croppedFile = new File([blob], origName, {
                        type: mimeType
                    });
