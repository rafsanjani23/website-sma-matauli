...
@section('content')
    <div class="max-w-4xl">
        <div class="mb-6">
            <a href="{{ route('admin.beranda.program-ib.index', request()->query()) }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-red-800 transition">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            ...
            <form action="{{ route('admin.beranda.program-ib.update', array_merge(['id' => $item->id], request()->query())) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
