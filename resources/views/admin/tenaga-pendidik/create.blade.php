...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            <h2 class="text-lg font-bold text-gray-900 mb-6">Tambah Tenaga Pendidik</h2>
            @if ($errors->any())
            ...
            @endif
            <form action="{{ route('admin.tenaga-pendidik.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                <div>
                ...
                <div>
                    ...
                    <select name="kategori" id="kategori" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition">
                        ...
                        <option value="Guru Mata Pelajaran" {{ old('kategori') == 'Guru Mata Pelajaran' ? 'selected' : '' }}>Guru Mata Pelajaran</option>
                        <option value="IB" {{ old('kategori') == 'IB' ? 'selected' : '' }}>IB</option>
                    </select>
                </div>
                <div class="pt-4">
