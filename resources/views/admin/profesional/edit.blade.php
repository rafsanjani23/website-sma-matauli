...
@section('content')
    <div class="max-w-4xl">
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6 md:p-8">
            ...
            @endif
            <form action="{{ route('admin.profesional.update', array_merge(['id' => $item->id], request()->query())) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                @csrf
                @method('PUT')
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="angkatan" id="angkatan" value="{{ old('angkatan', $item->angkatan) }}" placeholder="Contoh: 15" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-800 focus:border-transparent transition" />
                </div>
                <div>
