    ...
    <div class="max-w-7xl mx-auto px-4">
        <div class="mb-20 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse ($items as $item)
                <a href="{{ route('pengumuman-asrama-detail', $item->id) }}"
                    ...
                    <div class="p-5 md:p-6">
                        ...
                        <div class="flex items-center justify-between gap-3 text-xs">
                            <span class="text-slate-500">{{ $item->tanggal->format('d F Y') }}</span>
                            @if ($item->link_facebook)
                                <span class="inline-flex items-center gap-2 text-blue-600 group-hover:text-blue-700 font-semibold transition">
            ...
            @empty
                <div class="col-span-full text-center py-12 text-gray-400">
                    {{ __('Belum ada pengumuman asrama.') }}
                </div>
            @endforelse
        </div>
    </div>
