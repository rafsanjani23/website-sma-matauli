<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-xl border border-gray-200 overflow-x-auto">
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                @php $no = 1 + (($kedinasan->currentPage() - 1) * $kedinasan->perPage()) @endphp
                @forelse ($kedinasan as $item)
    ...
    @include('partials.pagination',['items' => $kedinasan])
    <!-- @if ($kedinasan->lastPage() > 1)
        <ul class="flex space-x-2 justify-center mt-6">
            <li>
                <a href="{{ $kedinasan->previousPageUrl() ?? '#' }}"
                    class="flex items-center justify-center shrink-0 bg-gray-100 w-9 h-9 rounded-md {{ $kedinasan->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
            ...
            @for ($i = 1; $i <= $kedinasan->lastPage(); $i++)
                <li>
                    <a href="{{ $kedinasan->url($i) }}"
                        class="flex items-center justify-center shrink-0 border {{ $kedinasan->currentPage() == $i ? 'bg-red-800 text-white border-red-800' : 'border-gray-200 hover:border-red-600 text-gray-900' }} cursor-pointer text-base font-medium px-3.25 h-9 rounded-md">
                        {{ $i }}
                    </a>
                </li>
            @endfor
            <li>
                <a href="{{ $kedinasan->nextPageUrl() ?? '#' }}"
                    class="flex items-center justify-center shrink-0 border border-gray-200 hover:border-red-600 w-9 h-9 rounded-md {{ !$kedinasan->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
