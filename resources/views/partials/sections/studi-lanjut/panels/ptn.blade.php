    ...
    @if ($ptn->lastPage() > 1)
        <ul class="flex space-x-2 justify-center mt-6">
            ...
            @for ($i = 1; $i <= $ptn->lastPage(); $i++)
                <li>
                    <a href="{{ $ptn->url($i) }}"
                        class="flex items-center justify-center shrink-0 border {{ $ptn->currentPage() == $i ? 'bg-red-800 text-white border-red-800' : 'border-gray-200 hover:border-red-600 text-gray-900' }} cursor-pointer text-base font-medium px-[13px] h-9 rounded-md">
                        {{ $i }}
                    </a>
                </li>
            @endfor
