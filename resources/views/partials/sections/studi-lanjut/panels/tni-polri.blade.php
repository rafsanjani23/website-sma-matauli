            ...
            @for ($i = 1; $i <= $tniPolri->lastPage(); $i++)
                <li>
                    <a href="{{ $tniPolri->url($i) }}"
                        class="flex items-center justify-center shrink-0 border {{ $tniPolri->currentPage() == $i ? 'bg-red-800 text-white border-red-800' : 'border-gray-200 hover:border-red-600 text-gray-900' }} cursor-pointer text-base font-medium px-3.25 h-9 rounded-md">
                        {{ $i }}
                    </a>
                </li>
            @endfor
