<div class="max-w-4xl mx-auto">
    ...
    @if ($tniPolri->lastPage() > 1)
        <ul class="flex space-x-2 justify-center mt-6">
            <li>
                <a href="{{ $tniPolri->previousPageUrl() ?? '#' }}"
                    class="flex items-center justify-center shrink-0 bg-gray-100 w-9 h-9 rounded-md {{ $tniPolri->onFirstPage() ? 'opacity-50 pointer-events-none' : '' }}">
            ...
            @for ($i = 1; $i <= $tniPolri->lastPage(); $i++)
                <li>
                    <a href="{{ $tniPolri->url($i) }}"
                        class="flex items-center justify-center shrink-0 border {{ $tniPolri->currentPage() == $i ? 'bg-red-800 text-white border-red-800' : 'border-gray-200 hover:border-red-600 text-gray-900' }} cursor-pointer text-base font-medium px-[13px] h-9 rounded-md">
                        {{ $i }}
                    </a>
                </li>
            @endfor
            <li>
                <a href="{{ $tniPolri->nextPageUrl() ?? '#' }}"
                    class="flex items-center justify-center shrink-0 border border-gray-200 hover:border-red-600 w-9 h-9 rounded-md {{ !$tniPolri->hasMorePages() ? 'opacity-50 pointer-events-none' : '' }}">
